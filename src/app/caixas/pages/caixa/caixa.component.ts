...
export class CaixaComponent implements OnInit {
  ...
  public novo(): void {
    const url = `${AppRoutes.Caixa.Cadastro()}/novo`;
  ...
  private pesquisar(pesquisa: string): void {
    this.loading = true;
    this.caixaService.get(pesquisa).subscribe(
      (result) => {
        this.caixas = result;
        this.loading = false;
      },
  ...
  public editar(caixa: Caixa): void {
    var url = `${AppRoutes.Caixa.Cadastro()}/${caixa.id}`;
    this.router.navigateByUrl(url);
  }

  public excluir(caixa: Caixa): void {
    if (confirm(`Deseja excluir o registro ${caixa.id}?`)) {
      this.caixaService.delete(caixa.id).subscribe(
        (result) => {
          this.pesquisar("");
        },
        (err) => {
